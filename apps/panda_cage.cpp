...
std::optional<DrrtDefaultParameters> tunedDrrtDefaults(int num_robots) {
    if (num_robots == 4)
        return DrrtDefaultParameters{300, 32};
    return std::nullopt;
}
...
struct CageBox {
    ...
    Eigen::Vector3d lo() const { return center - half_extents; }
    Eigen::Vector3d hi() const { return center + half_extents; }
};
...
void applyCageLineScale(json &doc, double scale) {
    if (std::abs(scale - 1.0) < 1e-12)
        return;
    if (scale <= 0.0)
        throw std::runtime_error("--cage-line-scale must be positive");
    if (!doc.contains("cage") || !doc.at("cage").contains("center") ||
        !doc.at("cage").contains("size") || !doc.contains("robot_bases")) {
        throw std::runtime_error(
            "--cage-line-scale requires cage center/size and robot_bases");
    }

    const Eigen::Vector3d cage_center =
        vector3FromJson(doc.at("cage").at("center"));
    Eigen::Vector3d cage_size = vector3FromJson(doc.at("cage").at("size"));
    const double old_half_y = cage_size.y() * 0.5;
    if (old_half_y <= 0.0)
        throw std::runtime_error("--cage-line-scale requires positive cage y size");

    auto &bases = doc.at("robot_bases");
    std::vector<double> old_base_y;
    std::vector<double> new_base_y;
    old_base_y.reserve(bases.size());
    new_base_y.reserve(bases.size());

    double old_outer_offset_y = 0.0;
    for (const auto &base : bases) {
        const double y = base.at("position").at(1).get<double>();
        old_base_y.push_back(y);
        old_outer_offset_y =
            std::max(old_outer_offset_y, std::abs(y - cage_center.y()));
    }

    const double end_margin_y = old_half_y - old_outer_offset_y;
    if (end_margin_y < -1e-9) {
        throw std::runtime_error(
            "--cage-line-scale cannot preserve a negative cage end margin");
    }

    double new_outer_offset_y = 0.0;
    for (std::size_t i = 0; i < bases.size(); ++i) {
        const double y =
            cage_center.y() + scale * (old_base_y[i] - cage_center.y());
        bases[i]["position"][1] = y;
        new_base_y.push_back(y);
        new_outer_offset_y =
            std::max(new_outer_offset_y, std::abs(y - cage_center.y()));
    }

    const double new_half_y =
        new_outer_offset_y + std::max(0.0, end_margin_y);
    cage_size.y() = 2.0 * new_half_y;
    doc["cage"]["size"][1] = cage_size.y();
    doc["cage_line_scale"] = scale;
    doc["cage_line_original_y_half_extent"] = old_half_y;
    doc["cage_line_y_half_extent"] = new_half_y;
    doc["cage_line_preserved_end_margin_y"] = end_margin_y;

    const auto remapCageY = [&](double y) {
        const double offset = y - cage_center.y();
        const double endpoint_eps = std::max(1e-9, old_half_y * 1e-9);
        if (std::abs(std::abs(offset) - old_half_y) <= endpoint_eps) {
            const double sign = offset < 0.0 ? -1.0 : 1.0;
            return cage_center.y() + sign * new_half_y;
        }
        return cage_center.y() + scale * offset;
    };

    if (doc.contains("obstacles")) {
        for (auto &obstacle : doc.at("obstacles")) {
            if (obstacle.contains("center") &&
                obstacle.at("center").is_array() &&
                obstacle.at("center").size() >= 2) {
                obstacle["center"][1] =
                    remapCageY(obstacle.at("center").at(1).get<double>());
            }
            if (obstacle.value("type", "") == "cylinder" &&
                obstacle.contains("axis") && obstacle.contains("half_height")) {
                const Eigen::Vector3d axis = vector3FromJson(obstacle.at("axis"));
                if (std::abs(axis.y()) > 0.5) {
                    obstacle["half_height"] =
                        obstacle.at("half_height").get<double>() *
                        (new_half_y / old_half_y);
                }
            }
        }
    }

    const auto adjustTargets = [&](json &targets) {
        if (!targets.is_array())
            return;
        const std::size_t count = std::min(targets.size(), new_base_y.size());
        for (std::size_t r = 0; r < count; ++r) {
            if (!targets[r].is_array() || targets[r].size() < 2)
                continue;
            targets[r][1] =
                targets[r].at(1).get<double>() + new_base_y[r] - old_base_y[r];
        }
    };

    if (doc.contains("tasks")) {
        for (auto &task : doc.at("tasks")) {
            if (task.contains("start_targets"))
                adjustTargets(task.at("start_targets"));
            if (task.contains("goal_targets"))
                adjustTargets(task.at("goal_targets"));
        }
    }
}
...
bool intersectionAABB(const CageBox &cage, const Eigen::Vector3d &sphere_center,
                      double sphere_radius, Eigen::Vector3d &lo,
                      Eigen::Vector3d &hi) {
    const Eigen::Vector3d slo = sphere_center.array() - sphere_radius;
    const Eigen::Vector3d shi = sphere_center.array() + sphere_radius;
    lo = cage.lo().cwiseMax(slo);
    hi = cage.hi().cwiseMin(shi);
    return (lo.array() < hi.array()).all();
}

Eigen::Vector3d sampleInIntersection(const CageBox &cage,
                                     const Eigen::Vector3d &base_pos,
                                     double reach_radius, std::mt19937 &rng) {
    Eigen::Vector3d lo;
    Eigen::Vector3d hi;
    if (!intersectionAABB(cage, base_pos, reach_radius, lo, hi))
        throw std::runtime_error(
            "Cage and robot reachable volume do not intersect");
    ...
    for (int i = 0; i < 100000; ++i) {
        Eigen::Vector3d pt(dx(rng), dy(rng), dz(rng));
        if ((pt - base_pos).norm() <= reach_radius)
            return pt;
    }
    throw std::runtime_error(
        "Failed to sample a point in cage/reachable-volume intersection");
}

TaskConfigs sampleConfigSet(
    const std::vector<std::shared_ptr<comotion::RobotModel>> &robots,
    const std::vector<Eigen::Vector3d> &base_positions,
    const std::vector<comotion::IKSolver> &ik_solvers,
    const comotion::CollisionChecker &checker, const CageBox &cage,
    double reach_radius, const std::string &ee_link, std::mt19937 &rng,
    int max_ik_attempts, int max_restarts) {
    ...
    for (int restart = 0; restart < max_restarts; ++restart) {
        ...
        for (int r = 0; r < num_robots; ++r) {
            ...
            for (int attempt = 0; attempt < max_ik_attempts; ++attempt) {
                Eigen::Vector3d target = sampleInIntersection(
                    cage, base_positions[static_cast<std::size_t>(r)],
                    reach_radius, rng);
    ...
    throw std::runtime_error(
        "Failed to sample a full collision-free Panda cage config set");
}
...
json generateTasksDoc(const AppOptions &options) {
    ...
    applyCageLineScale(doc, options.cage_line_scale);
    ...
    const int max_ik_attempts = options.max_ik_attempts.value_or(
        doc.value("max_ik_attempts", options.num_robots <= 2 ? 100 : 400));
    const int max_restarts = options.max_restarts.value_or(
        doc.value("max_restarts", options.num_robots <= 2 ? 100 : 400));

    CageBox cage;
    cage.center = vector3FromJson(doc.at("cage").at("center"));
    cage.half_extents = vector3FromJson(doc.at("cage").at("size")) * 0.5;
    ...
    base_positions.reserve(base_transforms.size());
    for (const auto &base : base_transforms)
        base_positions.push_back(base.translation());
    ...
    for (int t = 0; t < options.num_tasks; ++t) {
        std::cout << "Sampling Panda cage task " << (t + 1) << "/"
                  << options.num_tasks << "..." << std::flush;
        TaskConfigs starts =
            sampleConfigSet(robots, base_positions, ik_solvers, checker, cage,
...
GeneratedScenario loadScenarioFromDoc(const json &doc, const AppOptions &options,
    ...
    generated.cage = doc.value("cage", json::object());
...
json benchmarkContextJson(const GeneratedScenario &generated,
    ...
    return json{
        ...
        {"cage", generated.cage},
        {"cage_line_scale", options.cage_line_scale},
        {"obstacles",
...
std::string outputBasename(const GeneratedScenario &generated,
                           const AppOptions &options) {
    return "panda_cage_n" + std::to_string(generated.num_robots) + "_task" +
...
void printUsage(const char *prog) {
    std::cout
        << "Usage: " << prog
        << " --num-robots <2|4|8|16> [--task-index <i>] [options]\n"
        << "  --num-robots <N>       Built-in/custom task robot count\n"
        << "  --task-index <i>       Zero-based task index (default: 0)\n"
        << "  --task-file <path>     Read custom/generated Panda cage task JSON\n"
        ...
        << "  --cage-line-scale <s>  Scale robot base spacing along cage y-lines,\n"
        << "                         preserving cage end margin (default: 1.0)\n"
        ...
        << "  --time-limit <sec>     Planning time limit (default: 100)\n"
...
AppOptions parseArgs(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--cage-line-scale") {
            options.cage_line_scale =
                std::stod(requireValue(i, argc, argv, arg));
    ...
    if (options.num_robots != 2 && options.num_robots != 4 &&
        options.num_robots != 8 && options.num_robots != 16)
        throw std::runtime_error("--num-robots must be one of 2, 4, 8, or 16");
    if (const auto tuned = tunedDrrtDefaults(options.num_robots)) {
...
int main(int argc, char **argv) {
    try {
        ...
        if (options.generate_tasks_json) {
            ...
            std::cout << "Wrote " << options.num_tasks << " Panda cage task(s) to "
            ...
            applyCageLineScale(task_doc, options.cage_line_scale);
